...
uint32_t ppc_invert32(uint32_t v, uint32_t n)
{
	uint32_t res = 0;
	int i = 0;

	/* !!DEBUG!! */
	//printf("ppc_invert32(0X%08X, 0X%08X) => ", v, n);
...
uint64_t ppc_invert64(uint64_t v, uint64_t n)
{
	uint64_t res = 0;
	int i = 0;

	/* !!DEBUG!! */
	//printf("ppc_invert64(0X%016LX, 0X%016LX) => ", v, n);
...
float ppc_invertf(float v, uint32_t n)
{
	*((uint32_t *)&v) = ppc_invert32(*(uint32_t *)&v, n);
	return v;
}

double ppc_invertd(double v, uint32_t n)
{
	*((uint64_t *)&v) = ppc_invert64(*(uint64_t *)&v, n);
	return v;
}

/* for these functions no inversion is done and l <= u */
#ifdef COMPAT
uint32_t ppc_set_field32u(uint32_t v, uint32_t s, int32_t u, int32_t l) {
...
uint64_t ppc_set_field64u(uint64_t v, uint64_t s, int32_t u, int32_t l) {
	...
	/* !!DEBUG!! */
	//printf("ppc_set_field64u(0X%016llX, 0X%016llX, 0X%08X, 0X%08X) => ", v, s, u, l);
	//printf("0X%016llX\n", (v & ~mask) | ((s << l) & mask));
...
float ppc_set_fieldf(float v, uint32_t s, int32_t u, int32_t l) {
	...
	x.i = ppc_set_field32u(*((uint32_t *)&v), s, u, l);
	return x.d;
}
...
double ppc_set_fieldd(double v, uint64_t s, int32_t u, int32_t l) {
	...
	x.i = ppc_set_field64u(*((uint64_t *)&v), s, u, l);
	return x.d;
}
...
uint32_t ppc_set_field32u_inverted(uint32_t v, uint32_t s, int32_t u, int32_t l) {
	uint32_t mask = ppc_mask32(u - l + 1);
	assert(u >= l);

	/* !!DEBUG!! */
	//printf("ppc_set_field32u_inverted(0X%08X, 0X%08X, 0X%08X, 0X%08X) => ", v, s, u, l);
	//printf("0X%08X\n", (v & ~mask) | (ppc_invert32(s & mask, u-l+1) << l));

	return (v & ~(mask << l)) | (ppc_invert32(s, u-l+1) << l);
}

uint64_t ppc_set_field64u_inverted(uint64_t v, uint64_t s, int32_t u, int32_t l) {
	uint64_t mask = ppc_mask64(u - l + 1);

	/* !!DEBUG!! */
	//printf("ppc_set_field64u_inverted(0X%016llX, 0X%016llX, 0X%08X, 0X%08X) => ", v, s, u, l);
	//printf("0X%016llX\n", (v & ~mask) | (ppc_invert64(s & mask, u-l+1) << l));

	return (v & ~(mask << l)) | (ppc_invert64(s, u-l+1) << l);
}

float ppc_set_fieldf_inverted(float v, uint32_t s, int32_t u, int32_t l) {
	*((uint32_t *)&v) = ppc_set_field32u_inverted(*((uint32_t *)&v), s, u, l);
	return v;
}

double ppc_set_fieldd_inverted(double v, uint64_t s, int32_t u, int32_t l) {
	*((uint64_t *)&v) = ppc_set_field64u_inverted(*((uint64_t *)&v), s, u, l);
	return v;
}
...
{
	/* !!DEBUG!! */
	//printf("ppc_set_field32u_generic(0X%08X, 0X%08X, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, s, a, b, bit_order);

	/* only 1 bit to copy, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return ppc_set_field32u(v, s, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return ppc_set_field32u_inverted(v, s, b, a);
		else
			return ppc_set_field32u(v, s, b, a);
	}
	else
	{
		if (bit_order)
			...
			return ppc_set_field32_inverted(v, s, a, b);
	}
}
...
{
	/* !!DEBUG!! */
	//printf("ppc_set_field64u_generic(0X%016llX, 0X%016llX, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, s, a, b, bit_order);

	/* only 1 bit to copy, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return ppc_set_field64u(v, s, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return ppc_set_field64u_inverted(v, s, b, a);
		else
			return ppc_set_field64u(v, s, b, a);
	}
	else
	{
		if (bit_order)
			...
			return ppc_set_field64_inverted(v, s, a, b);
	}
}
...
uint32_t ppc_field32u_inverted(uint32_t v, uint32_t u, uint32_t l)
{
	...
	/* !!DEBUG!! */
	//printf("ppc_field32u_inverted(0X%08X, 0X%08X, 0X%08X) => ", v, u, l);
	//printf("0X%08X\n", ppc_invert32((v & mask) >> l, u-l+1));

	return ppc_invert32((v & mask) >> l, u-l+1);
}
...
uint64_t ppc_field64u_inverted(uint64_t v, uint32_t u, uint32_t l)
{
	...
	/* !!DEBUG!! */
	//printf("ppc_field64u_inverted(0X%016LX, 0X%08X, 0X%08X) => ", v, u, l);
	//printf("0X%016LX\n", ppc_invert64((v & mask) >> l, u-l+1));

	return ppc_invert64((v & mask) >> l, u-l+1);
}


uint32_t ppc_fieldf_inverted(float v, uint32_t u, uint32_t l) {
	return ppc_field32u_inverted(*(uint32_t *)&v, u, l);
}

uint64_t ppc_fieldd_inverted(double v, uint32_t u, uint32_t l) {
	return ppc_field64u_inverted(*(uint64_t *)&v, u, l);
}
...
{
	/* !!DEBUG!! */
	//printf("ppc_field32u_generic(0X%08X, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, a, b, bit_order);

	/* only 1 bit to extract, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return ppc_field32u(v, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return ppc_field32u_inverted(v, b, a);
		else
			return ppc_field32u(v, b, a);
	}
	else
	{
		if (bit_order)
			...
			return ppc_field32_inverted(v, a, b);
	}
}
...
{
	/* !!DEBUG!! */
	//printf("ppc_field64u_generic(0X%016LX, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, a, b, bit_order);

	/* only 1 bit to extract, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return ppc_field64u(v, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return ppc_field64u_inverted(v, b, a);
		else
			return ppc_field64u(v, b, a);
	}
	else
	{
		if (bit_order)
...
int ppc_enumerate(int val, int max) {
	if(val >= max)
		ppc_panic("coercition to enumeration out of bounds");
	else
		return val;
}
 ...
 * @return	Double value.
 */
double ppc_cast_64tod(uint64_t i) {
	union {
		uint64_t i;
		double d;
	} x;
