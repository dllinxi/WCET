...
//#define PPC_MEM_SPY		// define it to get memory action spy feature
...
/**
 * @typedef ppc_memory_t
 * This type is used to represent a memory space.
 * @ingroup memory
 */
typedef struct ppc_memory_t
{
...
#	endif
#ifdef PPC_MEM_SPY
	ppc_mem_spy_t spy_fun;	/** spy function */
 ...
 */
static void ppc_mem_default_spy(ppc_memory_t *mem, ppc_address_t addr, ppc_size_t size, ppc_access_t access, void *data) {
}
#endif
...
ppc_memory_t* ppc_mem_new(void) {
	...
	/* initialize spy */
#	ifdef PPC_MEM_SPY
		mem->spy_fun = ppc_mem_default_spy;
		mem->spy_data = 0;
#	endif

	return (ppc_memory_t*) mem;
}
 ...
 */
static page_entry_t* mem_get_page(memory_64_t* mem, ppc_address_t addr)
{
    uint32_t      hash1;
    page_entry_t* tmp;
    page_entry_t* entry;
...
ppc_memory_t* ppc_mem_copy(ppc_memory_t* memory)
{
    ...
    memory_64_t* target = (memory_64_t*)ppc_mem_new();
 ...
 * @ingroup memory
 */
void ppc_mem_write(ppc_memory_t* memory, ppc_address_t address, void* buffer, size_t size)
{
    assert(size > 0);
	int i;
...
#	ifdef PPC_MEM_SPY
    	mem->spy_fun(mem, address, size, ppc_access_write, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
void ppc_mem_read(ppc_memory_t *memory, ppc_address_t address, void *buffer, size_t size)//////////////////////////////
{
    assert(size > 0);
	int i;
    uint32_t      offset = FMOD(address, MEM_PAGE_SIZE);
...
#	ifdef PPC_MEM_SPY
    	mem->spy_fun(mem, address, size, ppc_access_read, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
uint8_t ppc_mem_read8(ppc_memory_t *mem, ppc_address_t address)
{
    page_entry_t* pte    = mem_get_page(mem, address);
    ppc_address_t offset = FMOD(address, MEM_PAGE_SIZE);
    uint8_t r;
...
#	ifdef PPC_MEM_SPY
    	mem->spy_fun(mem, address, sizeof(r), ppc_access_read, mem->spy_data);
#	endif
    return r;
}
 ...
 * @ingroup memory
 */
uint16_t ppc_mem_read16(ppc_memory_t *mem, ppc_address_t address) {
...
#   endif
    ...
    if(!((offset & 0x1) | ((offset + 1) & MEM_PAGE_SIZE)))
        ...
        ppc_mem_read(mem, address, val.bytes, 2);
        r = val.half;
    }
 ...
 * @ingroup memory
 */
uint32_t ppc_mem_read32(ppc_memory_t *mem, ppc_address_t address)
{
...
#   endif
    ...
    else
    {
		...
		ppc_mem_read(mem, address, val.bytes, 4);
        r = val.word;
    }
 ...
 * @ingroup memory
 */
uint64_t ppc_mem_read64(ppc_memory_t *mem, ppc_address_t address) {
...
#   if HOST_ENDIANNESS != TARGET_ENDIANNESS
    ...
    else
    {
        ...
        ppc_mem_read(mem, address, val.bytes, 8);
        r = val.dword;
    }
 ...
 * @ingroup memory
 */
float ppc_mem_readf(ppc_memory_t *mem, ppc_address_t address)
{
	union {
		uint32_t i;
		float f;
	} val;
    val.i = ppc_mem_read32(mem, address);
	return val.f;
}
 ...
 * @ingroup memory
 */
double ppc_mem_readd(ppc_memory_t *mem, ppc_address_t address)
{
	union {
		uint64_t i;
		double f;
	} val;
    val.i = ppc_mem_read64(mem, address);
	return val.f;
}
 ...
 * @ingroup memory
 */
long double ppc_mem_readld(ppc_memory_t *memory, ppc_address_t address)
{
 ...
 * @ingroup memory
 */
void ppc_mem_write8(ppc_memory_t* mem, ppc_address_t address, uint8_t val)
{
    ppc_address_t offset;
    page_entry_t*   pte;
    pte    = mem_get_page(mem, address);
...
#	ifdef PPC_MEM_SPY
    	mem->spy_fun(mem, address, sizeof(val), ppc_access_write, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
void ppc_mem_write16(ppc_memory_t* mem, ppc_address_t address, uint16_t val)
{
    ppc_address_t offset;
    uint16_t* q;
...
# endif
    ...
    if(!((offset & 0x1) | ((offset + 1) & MEM_PAGE_SIZE)))
        *q = val;
		...
		ppc_mem_write(mem, address, p->bytes, 2);
	}
 ...
 * @ingroup memory
 */
void ppc_mem_write32(ppc_memory_t* mem, ppc_address_t address, uint32_t val) {
	uint32_t* q;
    ppc_address_t offset;
    page_entry_t*   pte;
...
#   endif
    ...
    if(!((offset & 0x3) | ((offset + 3) & MEM_PAGE_SIZE)))
        *q = val;
        ...
        ppc_mem_write(mem, address, p->bytes, 4);
	}
 ...
 * @ingroup memory
 */
void ppc_mem_write64(ppc_memory_t *mem, ppc_address_t address, uint64_t val)
{
    ppc_address_t offset;
    uint64_t* q;
...
#   endif
    ...
    if(!((offset & 0x7) | ((offset + 7) & MEM_PAGE_SIZE)))
        *q = val;
        ...
        ppc_mem_write(mem, address, p->bytes, 8);
	}
 ...
 * @ingroup memory
 */
void ppc_mem_writef(ppc_memory_t *memory, ppc_address_t address, float val)
{
	union {
		uint32_t i;
		float f;
	...
	v.f = val;
    ppc_mem_write32(memory, address, v.i);
}
 ...
 * @ingroup memory
 */
void ppc_mem_writed(ppc_memory_t *memory, ppc_address_t address, double val)
{
	union {
		uint64_t i;
		double f;
	...
	v.f = val;
    ppc_mem_write64(memory, address, v.i);
}
 ...
 * @ingroup memory
 */
void ppc_mem_writeld(ppc_memory_t *memory, ppc_address_t address, long double val)
{
	assertp(0, "not implemented");
}
