 ...
 */
void copy_options_to_ppc_env(ppc_env_t *env, init_options *opt)
{
	env->argc = opt->argc;
...
FILE* load_profiling_file(char* profiling_file_name, int inst_stat[])
{
    ...
    if(profile_id == NULL)
    {
        ...
        for(i = 0; i<PPC_INSTRUCTIONS_NB; i++)
...
void write_profiling_file(FILE* profile_id, int inst_stat[])
{
	...
	}tmp, entries[PPC_INSTRUCTIONS_NB];
	...
	for(i = 0; i<PPC_INSTRUCTIONS_NB; i++)
	{
		for(j = i+1; j<PPC_INSTRUCTIONS_NB; j++)
		{
    ...
    for(i = 0; i<PPC_INSTRUCTIONS_NB; i++)
    {
        fprintf(profile_id, "%s %d %d\n",
        	ppc_get_string_ident(entries[i].id),	// instruction name
...
int main(int argc, char **argv) {
    ...
    FILE *profile_id=0;
    int inst_stat[PPC_INSTRUCTIONS_NB];
	...
	/* open the exec file */
    loader = ppc_loader_open(argv[prog_index]);
	if(loader == NULL) {
	...
	if (!is_exit_given) {
		...
		/* search symbol _exit */
        for(sym_exit = 0; sym_exit < ppc_loader_count_syms(loader); sym_exit++) {
            ppc_loader_sym_t data;
            ppc_loader_sym(loader, sym_exit, &data);
    ...
    copy_options_to_ppc_env(ppc_get_sys_env(platform), &options);

	/* load the image in the platform */
    if (ppc_load_platform(platform, argv[prog_index]) == -1) {
    ...
    if(profile)
    {
        ...
        if( profiling_file_path != NULL){
			profiling_file_name = (char*)alloca(sizeof(char)* (strlen(profiling_file_path) + strlen(PPC_PROC_NAME) + 10));
			...
			strcat(profiling_file_name, PPC_PROC_NAME);
		}else{
			profiling_file_name = (char*)alloca(sizeof(char)* strlen(PPC_PROC_NAME) + 10);
			strcpy(profiling_file_name, PPC_PROC_NAME);
		}
...
#	endif
    ...
    if(!verbose && !profile)
    {
			...
			{
				while(addr_exit != state->PPC_PC_NAME)
				{
					ppc_step(sim);
					inst_cnt++;
				}
